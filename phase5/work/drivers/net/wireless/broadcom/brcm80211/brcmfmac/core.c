...
int __init brcmf_core_init(void)
{
	...
	pr_emerg("BCM4360 test.153: brcmf_core_init() entry\n");
	pr_emerg("BCM4360 test.153: before brcmf_sdio_register()\n");
	err = brcmf_sdio_register();
	pr_emerg("BCM4360 test.153: after brcmf_sdio_register() err=%d\n", err);
	if (!err)
	...
	mdelay(50);
	pr_emerg("BCM4360 test.153: post-SDIO sync (before PCI)\n");
	pr_emerg("BCM4360 test.153: before brcmf_pcie_register()\n");
	err = brcmf_pcie_register();
	pr_emerg("BCM4360 test.153: after brcmf_pcie_register() err=%d\n", err);
	if (!err)
	...
	mdelay(50);
	pr_emerg("BCM4360 test.153: post-PCI sync (skipping USB)\n");
	return 0;
}
