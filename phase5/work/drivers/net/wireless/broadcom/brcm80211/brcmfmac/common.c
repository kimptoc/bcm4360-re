...
static int __init brcmfmac_module_init(void)
{
	...
	/* test.149: log module_init entry before the PCI registration window.
	...
	pr_emerg("BCM4360 test.149: before brcmf_core_init()\n");
	err = brcmf_core_init();
	pr_emerg("BCM4360 test.149: after brcmf_core_init() err=%d\n", err);
	if (err) {
		if (brcmfmac_pdata)
