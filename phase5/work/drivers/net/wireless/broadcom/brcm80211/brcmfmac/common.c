...
static int __init brcmfmac_module_init(void)
{
	...
	/* test.154: log module_init entry before the PCI registration window.
	...
	pr_emerg("BCM4360 test.154: before brcmf_core_init()\n");
	err = brcmf_core_init();
	pr_emerg("BCM4360 test.154: after brcmf_core_init() err=%d\n", err);
	if (err) {
		if (brcmfmac_pdata)
