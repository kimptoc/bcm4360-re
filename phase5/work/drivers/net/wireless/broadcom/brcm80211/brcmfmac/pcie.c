					  ...
					  const char *tag)
{
	 ...
	 * test.186c adds IOSTATUS (0x40c) to expose firmware-visible wrapper bits. */
	...
	pr_emerg("BCM4360 test.186c: %s ARM CR4 IOCTL=0x%08x IOSTATUS=0x%08x RESET_CTL=0x%08x CPUHALT=%s\n",
		 tag, ioctl, iostat, rstctl, (ioctl & 0x20) ? "YES" : "NO");
}


/* test.186c: read-only probe of D11 (BCMA_CORE_80211) wrapper.
 *
				       ...
				       const char *tag)
{
	...
	brcmf_pcie_select_core(devinfo, BCMA_CORE_CHIPCOMMON);
	pr_emerg("BCM4360 test.186c: %s D11 IOCTL=0x%08x IOSTATUS=0x%08x RESET_CTL=0x%08x\n",
		 tag, ioctl, iostat, rstctl);
}


/* test.186c: snapshot ChipCommon backplane registers that firmware would
					...
					u32 nvram_len)
{
	...
	if (devinfo->pdev->device == BRCM_PCIE_4360_DEVICE_ID) {
		...
		u32 pre_wide[40] = {0};	/* test.186c: 16-KB grid across 640-KB TCM */
		u32 pre_tail[16] = {0};	/* test.186c: last 64 B of TCM */
		u32 pre_bp[BRCMF_BP_REG_COUNT] = {0};	/* test.186c: backplane regs */
		...
		pr_emerg("BCM4360 test.186c: re-halting ARM CR4 via brcmf_chip_set_passive\n");
		mdelay(50);
		...
		pr_emerg("BCM4360 test.186c: starting chunked fw write, total_words=%u (%zu bytes) tail=%u wbase=%px\n",
		...
		for (i = 0; i < total_words; i++) {
			...
			if ((i + 1) % chunk_words == 0) {
				pr_emerg("BCM4360 test.186c: wrote %u words (%u bytes)\n",
					 i + 1, (i + 1) * 4);
				mdelay(50);
			}
		}

		pr_emerg("BCM4360 test.186c: all %u words written, before tail (tail=%u)\n",
			 total_words, tail);
		mdelay(50);
		...
		if (tail) {
			...
			pr_emerg("BCM4360 test.186c: tail %u bytes written at offset %zu\n",
				 tail, fw->size & ~3u);
			mdelay(50);
		}

		pr_emerg("BCM4360 test.186c: fw write complete (%zu bytes)\n",
			 fw->size);
		/* test.186c: test.181 proved brcmf_chip_set_active(ci, resetintr)
		 ...
		 */
		pr_emerg("BCM4360 test.186c: before post-fw msleep(100)\n");
		msleep(100);
		pr_emerg("BCM4360 test.186c: after post-fw msleep(100)\n");
		resetintr = get_unaligned_le32(fw->data);
		pr_emerg("BCM4360 test.186c: host resetintr=0x%08x before NVRAM\n",
			 resetintr);
		...
		if (nvram) {
			...
			pr_emerg("BCM4360 test.186c: pre-NVRAM write address=0x%x len=%u naddr=%px\n",
			...
			for (j = 0; j < nwords; j++) {
				iowrite32(le32_to_cpu(nsrc32[j]),
					  naddr + j * 4);
				if ((j + 1) % nchunk == 0) {
					pr_emerg("BCM4360 test.186c: NVRAM wrote %u words (%u bytes)\n",
						 j + 1, (j + 1) * 4);
					mdelay(50);
				}
			}
			...
			pr_emerg("BCM4360 test.186c: post-NVRAM write done (%u bytes)\n",
				 nvram_len);
			sharedram_addr_written =
				brcmf_pcie_read_ram32(devinfo,
						       devinfo->ci->ramsize - 4);
			pre_marker = sharedram_addr_written;
			pr_emerg("BCM4360 test.186c: NVRAM marker at ramsize-4 = 0x%08x (pre-release snapshot)\n",
				 sharedram_addr_written);
			for (j = 0; j < 8; j++) {
				u32 offset = j * 4;
				...
				pre_tcm[j] = val;
				pr_emerg("BCM4360 test.186c: TCM[0x%04x]=0x%08x (pre-release snapshot)\n",
					 offset, val);
			}
			/* test.186c: 16-KB wide grid across the full 640-KB TCM.
			...
			for (j = 0; j < ARRAY_SIZE(wide_offsets); j++) {
				...
				pre_wide[j] = val;
				pr_emerg("BCM4360 test.186c: wide-TCM[0x%05x]=0x%08x (pre-release snapshot)\n",
					 offset, val);
			}
			/* test.186c: last 64 bytes of TCM cover the sharedram
			...
			for (j = 0; j < ARRAY_SIZE(pre_tail); j++) {
				...
				pre_tail[j] = val;
				pr_emerg("BCM4360 test.186c: tail-TCM[0x%05x]=0x%08x (pre-release snapshot)\n",
					 offset, val);
			}
			/* test.186c: backplane-register snapshot via ChipCommon.
			...
			for (j = 0; j < BRCMF_BP_REG_COUNT; j++)
				pr_emerg("BCM4360 test.186c: CC-%s=0x%08x (pre-release snapshot)\n",
					 brcmf_bp_reg_names[j], pre_bp[j]);
		} else {
			pr_emerg("BCM4360 test.186c: no NVRAM loaded before early return\n");
		}

		{
			...
			if (imem_core) {
				pr_emerg("BCM4360 test.186c: pre-resetcore INTERNAL_MEM core->base=0x%08x rev=%u\n",
					 imem_core->base, imem_core->rev);
				mdelay(50);
				brcmf_chip_resetcore(imem_core, 0, 0, 0);
				mdelay(50);
				pr_emerg("BCM4360 test.186c: post-resetcore INTERNAL_MEM complete\n");
			} else {
				pr_emerg("BCM4360 test.186c: INTERNAL_MEM core not found — resetcore skipped (expected on BCM4360)\n");
			}
		}

		{
			...
			brcmf_pcie_probe_d11_state(devinfo,
						   "pre-set-active");
			mdelay(50);
			pr_emerg("BCM4360 test.186c: calling brcmf_chip_set_active resetintr=0x%08x (BusMaster stays cleared)\n",
				 resetintr);
			mdelay(50);
			sa_rc = brcmf_chip_set_active(devinfo->ci,
						      resetintr);
			pr_emerg("BCM4360 test.186c: brcmf_chip_set_active returned %s\n",
				 sa_rc ? "true" : "false");
			mdelay(20);
			brcmf_pcie_probe_armcr4_state(devinfo,
			...
			/* test.186c: extended post-release observation.
			...
			for (d = 0; d < ARRAY_SIZE(dwell_labels_ms); d++) {
				...
				marker_now = brcmf_pcie_read_ram32(devinfo,
						devinfo->ci->ramsize - 4);
				pr_emerg("BCM4360 test.186c: dwell-%ums NVRAM marker=0x%08x (was 0x%08x) %s\n",
				...
				for (j = 0; j < 8; j++) {
					...
					pr_emerg("BCM4360 test.186c: dwell-%ums TCM[0x%04x]=0x%08x (was 0x%08x) %s\n",
				...
				for (j = 0; j < ARRAY_SIZE(wide_offsets); j++) {
					...
					pr_emerg("BCM4360 test.186c: dwell-%ums wide-TCM[0x%05x]=0x%08x (was 0x%08x) %s\n",
						 dwell_labels_ms[d], offset,
						 val, pre_wide[j],
						 val == pre_wide[j] ?
							"UNCHANGED" :
				...
				for (j = 0; j < ARRAY_SIZE(pre_tail); j++) {
					...
					pr_emerg("BCM4360 test.186c: dwell-%ums tail-TCM[0x%05x]=0x%08x (was 0x%08x) %s\n",
						 dwell_labels_ms[d], offset,
						 val, pre_tail[j],
						 val == pre_tail[j] ?
							"UNCHANGED" :
...

				{
					...
					for (j = 0; j < BRCMF_BP_REG_COUNT;
					     j++)
						pr_emerg("BCM4360 test.186c: dwell-%ums CC-%s=0x%08x (was 0x%08x) %s\n",
							 dwell_labels_ms[d],
		 ...
		 */
		{
			u32 mbint_pre, mbint_cleared;
			u32 mbint_after_mb0, mbint_after_mb1, mbint_after_sbmbx;
			static const u32 post_kick_labels_ms[] = { 500, 2000 };
			static const u32 post_kick_increments_ms[] = { 500, 1500 };
			u32 k;
			u32 j;
			...
			pr_emerg("BCM4360 test.186c: pre-kick PCIE2 mailboxint=0x%08x\n",
				 mbint_pre);

			/* test.186c: W1C-clear all mailboxint bits, then sample
			 * AFTER each individual kick to attribute bit changes
			 * precisely. brcmfmac's ISR uses the same W1C pattern —
			 * writing 0xffffffff clears every latched bit. */
			brcmf_pcie_write_reg32(devinfo,
					devinfo->reginfo->mailboxint,
					0xffffffff);
			mbint_cleared = brcmf_pcie_read_reg32(devinfo,
					devinfo->reginfo->mailboxint);
			pr_emerg("BCM4360 test.186c: after W1C-clear mailboxint=0x%08x (expected 0x0)\n",
				 mbint_cleared);
			mdelay(20);

			brcmf_pcie_write_reg32(devinfo,
					devinfo->reginfo->h2d_mailbox_0, 1);
			mbint_after_mb0 = brcmf_pcie_read_reg32(devinfo,
					devinfo->reginfo->mailboxint);
			pr_emerg("BCM4360 test.186c: kicked H2D_MAILBOX_0 (BAR0+0x%x)=1 -> mailboxint=0x%08x (delta=0x%08x)\n",
				 devinfo->reginfo->h2d_mailbox_0,
				 mbint_after_mb0,
				 mbint_after_mb0 ^ mbint_cleared);
			mdelay(20);

			brcmf_pcie_write_reg32(devinfo,
					devinfo->reginfo->h2d_mailbox_1, 1);
			mbint_after_mb1 = brcmf_pcie_read_reg32(devinfo,
					devinfo->reginfo->mailboxint);
			pr_emerg("BCM4360 test.186c: kicked H2D_MAILBOX_1 (BAR0+0x%x)=1 -> mailboxint=0x%08x (delta=0x%08x)\n",
				 devinfo->reginfo->h2d_mailbox_1,
				 mbint_after_mb1,
				 mbint_after_mb1 ^ mbint_after_mb0);
			mdelay(20);

			pci_write_config_dword(devinfo->pdev,
					       BRCMF_PCIE_REG_SBMBX, 1);
			brcmf_pcie_select_core(devinfo, BCMA_CORE_PCIE2);
			mbint_after_sbmbx = brcmf_pcie_read_reg32(devinfo,
					devinfo->reginfo->mailboxint);
			pr_emerg("BCM4360 test.186c: kicked SBMBX (PCI config 0x%x)=1 -> mailboxint=0x%08x (delta=0x%08x)\n",
				 BRCMF_PCIE_REG_SBMBX,
				 mbint_after_sbmbx,
				 mbint_after_sbmbx ^ mbint_after_mb1);

			for (k = 0; k < ARRAY_SIZE(post_kick_labels_ms); k++) {
				char tag[32];
				u32 marker_now, mbint_now;

				msleep(post_kick_increments_ms[k]);
				snprintf(tag, sizeof(tag),
					 "post-kick-%ums",
					 post_kick_labels_ms[k]);
				...
				mbint_now = brcmf_pcie_read_reg32(devinfo,
						devinfo->reginfo->mailboxint);
				pr_emerg("BCM4360 test.186c: %s PCIE2 mailboxint=0x%08x (post-sbmbx was 0x%08x) %s\n",
					 tag, mbint_now, mbint_after_sbmbx,
					 mbint_now == mbint_after_sbmbx ?
						"UNCHANGED" : "CHANGED");

				marker_now = brcmf_pcie_read_ram32(devinfo,
						devinfo->ci->ramsize - 4);
				pr_emerg("BCM4360 test.186c: %s NVRAM marker=0x%08x (was 0x%08x) %s\n",
				...
				for (j = 0; j < 8; j++) {
					...
					pr_emerg("BCM4360 test.186c: %s TCM[0x%04x]=0x%08x (was 0x%08x) %s\n",
						 tag, offset, val,
						 pre_tcm[j],
						 val == pre_tcm[j] ?
							"UNCHANGED" :
				...
				for (j = 0; j < ARRAY_SIZE(wide_offsets);
					...
					pr_emerg("BCM4360 test.186c: %s wide-TCM[0x%05x]=0x%08x (was 0x%08x) %s\n",
						 tag, offset, val,
						 pre_wide[j],
						 val == pre_wide[j] ?
							"UNCHANGED" :
				...
				for (j = 0; j < ARRAY_SIZE(pre_tail); j++) {
					...
					pr_emerg("BCM4360 test.186c: %s tail-TCM[0x%05x]=0x%08x (was 0x%08x) %s\n",
						 tag, offset, val,
						 pre_tail[j],
						 val == pre_tail[j] ?
							"UNCHANGED" :
...

				{
					...
					for (j = 0; j < BRCMF_BP_REG_COUNT;
					     j++)
						pr_emerg("BCM4360 test.186c: %s CC-%s=0x%08x (was 0x%08x) %s\n",
							 tag,
							 brcmf_bp_reg_names[j],
							 bp_now[j],
							 pre_bp[j],
		...
		pr_emerg("BCM4360 test.186c: released fw/nvram after H2D mailbox kick + post-kick sampling; returning -ENODEV\n");
		return -ENODEV;
	} else {
	...
	resetintr = get_unaligned_le32(fw->data);
	release_firmware(fw);
	pr_emerg("BCM4360 test.186c: after release_firmware resetintr=0x%08x\n",
		 resetintr);
	mdelay(50);
	...
	if (nvram) {
		address = devinfo->ci->rambase + devinfo->ci->ramsize -
			  nvram_len;
		pr_emerg("BCM4360 test.186c: pre-NVRAM write address=0x%x len=%u tcm=%px\n",
...
brcmf_pcie_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
	...
	if (pdev->device == BRCM_PCIE_4360_DEVICE_ID) {
		...
		msleep(300); /* test.186c: flush before root-port ASPM/CLKPM work */

		/* test.186c keeps the root-port LnkCtl logging from test.172 for
		...
		if (bridge) {
			pcie_capability_read_word(bridge, PCI_EXP_LNKCTL,
						  &rp_lnkctl_before);
			dev_emerg(&pdev->dev,
				  "BCM4360 test.186c: root port %s LnkCtl before=0x%04x ASPM=0x%x CLKREQ=%s — disabling L0s/L1/CLKPM\n",
			...
			pr_emerg("BCM4360 test.186c: root-port pci_disable_link_state returned — reading LnkCtl\n");
			msleep(300);

			pcie_capability_read_word(bridge, PCI_EXP_LNKCTL,
						  &rp_lnkctl_after);
			dev_emerg(&pdev->dev,
				  "BCM4360 test.186c: root port %s LnkCtl after=0x%04x ASPM=0x%x CLKREQ=%s\n",
			...
			dev_emerg(&pdev->dev,
				  "BCM4360 test.186c: no upstream bridge found; root-port ASPM/CLKPM disable skipped\n");
			msleep(300);
		}
...
{
	pr_emerg("BCM4360 test.186c: module_init entry — extended post-release TCM sampling\n");
}
...
int brcmf_pcie_register(void)
{
	...
	pr_emerg("BCM4360 test.186c: brcmf_pcie_register() entry\n");
	msleep(300); /* flush marker before pci_register_driver */
	pr_emerg("BCM4360 test.186c: before pci_register_driver\n");
	msleep(300); /* flush — if crash here, it's in pci_register_driver kernel code */
	ret = pci_register_driver(&brcmf_pciedrvr);
	pr_emerg("BCM4360 test.186c: pci_register_driver returned ret=%d\n", ret);
	return ret;
}
