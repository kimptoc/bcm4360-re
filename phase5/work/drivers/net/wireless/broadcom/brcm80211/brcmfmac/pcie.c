...
/* BCM4360 test.284: PRE-set_active MAILBOXMASK enable with persistence
 * tracking. T280 proved MBM writes silently fail AT post-set_active;
 * T241 proved MBM writes WORK at pre-set_active. T284 calls
 * brcmf_pcie_intr_enable BEFORE brcmf_chip_set_active and reads MBM
 * back at multiple stages through fw init to determine whether the
 * pre-set mask survives ARM release (home run) or gets reset at some
 * specific event. Readback cadence: pre-write, post-write (pre-set_active),
 * post-set_active, post-T276-poll, post-T278-initial-dump, plus each
 * T278 stage hook at t+500ms/t+5s/t+30s/t+90s. Requires T276+T277+T278.
 * Advisor trace 2026-04-24 post-T280. */
static int bcm4360_test284_premask_enable;
module_param(bcm4360_test284_premask_enable, int, 0644);
MODULE_PARM_DESC(bcm4360_test284_premask_enable, "BCM4360 test.284: call brcmf_pcie_intr_enable BEFORE brcmf_chip_set_active. Read MAILBOXMASK back at multiple stages (pre-write, post-write, post-set_active, post-T276-poll, post-T278-initial, and at each T278 stage hook) to track whether pre-set mask survives fw init. Requires T276+T277+T278. (1=enable, 0=off)");

/* BCM4360 test.284: MBM + MBXINT one-line readback helper.
 * Zero cost when gate flag is off. */
#define BCM4360_T284_READ_MBM(tag) do { \
	if (bcm4360_test284_premask_enable) { \
		u32 _t284_mbm = brcmf_pcie_read_reg32(devinfo, \
			BRCMF_PCIE_PCIE2REG_MAILBOXMASK); \
		u32 _t284_mbx = brcmf_pcie_read_reg32(devinfo, \
			BRCMF_PCIE_PCIE2REG_MAILBOXINT); \
		pr_emerg("BCM4360 test.284: %s MAILBOXMASK=0x%08x MAILBOXINT=0x%08x\n", \
			 tag, _t284_mbm, _t284_mbx); \
	} \
} while (0)
 ...
 * INTSTATUS at 0x18000100). T285 reads INTSTATUS (0x100), INTMASK
 * (0x104), and a candidate pending-events location (0x168) at each
 * T284 stage. Saves BAR0_WINDOW before switching to CHIPCOMMON and
 * restores after EACH read so other BAR0 accesses (assumed to be on
 * PCIE2) aren't silently corrupted. Requires T276+T277+T278+T284.
 * Advisor trace 2026-04-24 post-T283. READ-ONLY. */
static int bcm4360_test285_chipcommon_read;
module_param(bcm4360_test285_chipcommon_read, int, 0644);
MODULE_PARM_DESC(bcm4360_test285_chipcommon_read, "BCM4360 test.285: at each T284 stage, save BAR0_WINDOW, select_core(CHIPCOMMON), read INTSTATUS (0x100), INTMASK (0x104), 0x168, restore BAR0_WINDOW. READ-ONLY. Requires T276+T277+T278+T284. (1=enable, 0=off)");

/* BCM4360 test.285: chipcommon-read helper. Mandatory window
 * save/restore to avoid silent PCIE2-access corruption. */
#define BCM4360_T285_READ_CC(tag) do { \
	if (bcm4360_test285_chipcommon_read) { \
		u32 _saved_win, _cc_intstatus, _cc_intmask, _cc_0x168; \
		pci_read_config_dword(devinfo->pdev, \
			BRCMF_PCIE_BAR0_WINDOW, &_saved_win); \
		brcmf_pcie_select_core(devinfo, BCMA_CORE_CHIPCOMMON); \
		_cc_intstatus = brcmf_pcie_read_reg32(devinfo, 0x100); \
		_cc_intmask   = brcmf_pcie_read_reg32(devinfo, 0x104); \
		_cc_0x168     = brcmf_pcie_read_reg32(devinfo, 0x168); \
		pci_write_config_dword(devinfo->pdev, \
			BRCMF_PCIE_BAR0_WINDOW, _saved_win); \
		pr_emerg("BCM4360 test.285: %s CC.INTSTATUS=0x%08x CC.INTMASK=0x%08x CC[0x168]=0x%08x (saved_win=0x%08x)\n", \
...
/* BCM4360 test.298: BAR2-only ISR-list walk + sched_ctx/pending-events read.
 * Honours KEY_FINDINGS row 85 stopping rule: zero BAR0 / select_core /
 * chipcommon-wrap / PCIE2-wrap touches. Uses ONLY brcmf_pcie_read_ram32
 * (pcie.c:1875 — direct BAR2 ioread, no BAR0_WINDOW write).
 *
 * Static rationale in phase6/t298_static_pass_findings.md.
 *
 * Walks the linked list whose head ptr lives at TCM[0x629A4] (per T269/T289
 * disasm of hndrte_add_isr fn@0x63C24). Each 16-byte node:
 *   [+0]=next, [+4]=callback fn, [+8]=arg, [+0xC]=1<<bit_index
 * where bit_index was allocated by BIT_alloc reading wrap+0x100
 * (oobselouta30) at registration time. Reading [+0xC] gives us the OOB
 * bit allocation WITHOUT touching the wrapper register at runtime.
 *
 * Static enumeration found 3 hndrte_add_isr callers in the blob:
 *   fn=0x1C99 (pciedngl_isr) — caller pcidongle_probe @ 0x1F28; LIVE per T256
 *   fn=0x0B05 (RTE class-0x800 chipcommon ISR thunk) — caller @ 0x63CF0; likely LIVE
 *   fn=0x1146D (wlc_isr) — caller wl_attach @ 0x67774; DEAD in offload mode per T299/T306
 *
 * Also reads sched_ctx[+0xCC] (current class) and pending-events word at
 * *(*(sched+0x358))+0x100) for HW IRQ activity shadow. */
static int bcm4360_test298_isr_walk;
...
/* BCM4360 test.298: BAR2-only dynamic ISR-list walk + sched/pending read.
 * Defensive bounds: 0xA0000 = ramsize per T121. 16-node walk cap.
 * ZERO BAR0 touches — uses only brcmf_pcie_read_ram32 (BAR2-direct). */
#define BCM4360_T298_RAMSIZE_BOUND  0xA0000
#define BCM4360_T298_MAX_NODES      16
#define BCM4360_T298_LIST_HEAD_ADDR 0x629A4
#define BCM4360_T298_SCHED_PTR_ADDR 0x6296C

#define BCM4360_T298_ISR_WALK(tag) do { \
	if (bcm4360_test298_isr_walk) { \
		u32 _t298_head = brcmf_pcie_read_ram32(devinfo, \
			BCM4360_T298_LIST_HEAD_ADDR); \
		u32 _t298_sched = brcmf_pcie_read_ram32(devinfo, \
			BCM4360_T298_SCHED_PTR_ADDR); \
		u32 _t298_node = _t298_head; \
		u32 _t298_n; \
		u32 _t298_cc = 0xFFFFFFFF; \
		u32 _t298_evp = 0, _t298_pending = 0; \
		pr_emerg("BCM4360 test.298: %s head=0x%08x sched_ctx=0x%08x\n", \
			 tag, _t298_head, _t298_sched); \
		for (_t298_n = 0; _t298_n < BCM4360_T298_MAX_NODES; _t298_n++) { \
			u32 _t298_next, _t298_fn, _t298_arg, _t298_mask; \
			if (_t298_node == 0 || \
			    _t298_node >= BCM4360_T298_RAMSIZE_BOUND) { \
				pr_emerg("BCM4360 test.298: %s node[%u] end ptr=0x%08x\n", \
					 tag, _t298_n, _t298_node); \
				break; \
			} \
			_t298_next = brcmf_pcie_read_ram32(devinfo, \
				_t298_node + 0x0); \
			_t298_fn   = brcmf_pcie_read_ram32(devinfo, \
				_t298_node + 0x4); \
			_t298_arg  = brcmf_pcie_read_ram32(devinfo, \
				_t298_node + 0x8); \
			_t298_mask = brcmf_pcie_read_ram32(devinfo, \
				_t298_node + 0xC); \
			pr_emerg("BCM4360 test.298: %s node[%u]@0x%08x next=0x%08x fn=0x%08x arg=0x%08x mask=0x%08x\n", \
				 tag, _t298_n, _t298_node, _t298_next, \
				 _t298_fn, _t298_arg, _t298_mask); \
			_t298_node = _t298_next; \
		} \
		if (_t298_sched != 0 && \
		    _t298_sched < BCM4360_T298_RAMSIZE_BOUND) { \
			_t298_cc = brcmf_pcie_read_ram32(devinfo, \
				_t298_sched + 0xCC); \
			_t298_evp = brcmf_pcie_read_ram32(devinfo, \
				_t298_sched + 0x358); \
			if (_t298_evp != 0 && \
			    _t298_evp < BCM4360_T298_RAMSIZE_BOUND) \
				_t298_pending = brcmf_pcie_read_ram32(devinfo, \
					_t298_evp + 0x100); \
		} \
		pr_emerg("BCM4360 test.298: %s SUMMARY count=%u sched_cc=0x%08x events_p=0x%08x pending=0x%08x\n", \
			 tag, _t298_n, _t298_cc, _t298_evp, _t298_pending); \
	} \
} while (0)
 ...
 *   6. Restore original
 *   7. Read once more — verifies restore landed (proves write CAN work
 *      even if step 5 showed drop)
 *   8. Restore BAR0_WINDOW
 *
 * Discriminator:
 *   readback == 0xDEADBEEF → chipcommon writes work
 *   readback == orig       → writes silently dropped (same pattern as MBM)
 ...
 * match the post-poll seeding precondition. If T284 is also enabled,
 * the MBM read piggybacks (prefixed with the same stage tag) so the
 * MBM persistence time series lines up with console deltas. */
...
#define BCM4360_T243_WRITEVERIFY(ms_tag) do { \
				...
				/* BCM4360 test.284: pre-set_active MAILBOXMASK enable.
				 * Runs AFTER T276 shared_info write (if enabled) but
				 * BEFORE brcmf_chip_set_active. T280 proved post-
				 * set_active MBM writes silently fail; T241 proved
