...
static int brcmf_chip_cores_check(struct brcmf_chip_priv *ci)
{
	...
	list_for_each_entry(core, &ci->cores, list) {
		brcmf_err("BCM4360 test.208: core[%-2d] id=0x%x:rev%-3d base=0x%08x wrap=0x%08x\n",
	...
	brcmf_err("BCM4360 test.208: host-side enumerated %d cores total (firmware assert wants r6==9 — see test.207)\n",
		  idx - 1);
	if (!cpu_found) {
...
static int brcmf_chip_setup(struct brcmf_chip_priv *chip)
{
	...
	if (pub->chip == BRCM_CC_4360_CHIP_ID) {
		...
		brcmf_err("BCM4360 test.208: max_res_mask 0x%08x -> 0x%08x (write 0x17f — bit 6 only; extended core-table dump + host-side core count)\n",
