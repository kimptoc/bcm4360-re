...
static int brcmf_chip_cores_check(struct brcmf_chip_priv *ci)
{
	...
	list_for_each_entry(core, &ci->cores, list) {
		brcmf_err("BCM4360 test.217: core[%-2d] id=0x%x:rev%-3d base=0x%08x wrap=0x%08x\n",
	...
	brcmf_err("BCM4360 test.217: host-side enumerated %d cores total\n",
		  idx - 1);
	if (!cpu_found) {
...
static int brcmf_chip_setup(struct brcmf_chip_priv *chip)
{
	...
	if (pub->chip == BRCM_CC_4360_CHIP_ID) {
		...
		brcmf_err("BCM4360 test.217: max_res_mask 0x%08x -> 0x%08x (0x17f)\n",
		...
		brcmf_err("BCM4360 test.217: min_res_mask 0x%08x -> 0x%08x (force bit 2 on)\n",
