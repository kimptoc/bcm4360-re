...
mkdir -p "$LOG_DIR"
LOG="$LOG_DIR/test.120.stage${STAGE}"

echo "=== test.120: post-chip-attach probe setup stage test --- stage=$STAGE ===" | tee "$LOG"
...
echo "" | tee -a "$LOG"
echo "=== Loading brcmfmac (bcm4360_reset_stage=$STAGE, bcm4360_skip_arm=$SKIP_ARM) --- test.120 ===" | tee -a "$LOG"
sync
