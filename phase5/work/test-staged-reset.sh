...
mkdir -p "$LOG_DIR"
LOG="$LOG_DIR/test.121.stage${STAGE}"

echo "=== test.121: fixed BCM4360 RAM-info stage test --- stage=$STAGE ===" | tee "$LOG"
...
echo "" | tee -a "$LOG"
echo "=== Loading brcmfmac (bcm4360_reset_stage=$STAGE, bcm4360_skip_arm=$SKIP_ARM) --- test.121 ===" | tee -a "$LOG"
sync
