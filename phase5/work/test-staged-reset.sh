...
mkdir -p "$LOG_DIR"
LOG="$LOG_DIR/test.122.stage${STAGE}"

echo "=== test.122: bypass BCM4360 reset_device stage test --- stage=$STAGE ===" | tee "$LOG"
...
echo "" | tee -a "$LOG"
echo "=== Loading brcmfmac (bcm4360_reset_stage=$STAGE, bcm4360_skip_arm=$SKIP_ARM) --- test.122 ===" | tee -a "$LOG"
sync
