...
mkdir -p "$LOG_DIR"
LOG="$LOG_DIR/test.176.stage${STAGE}"

echo "=== test.176: host resetintr read after post-fw msleep — stage=$STAGE ===" | tee "$LOG"
echo "Date: $(date)" | tee -a "$LOG"
echo "" | tee -a "$LOG"

case "$STAGE" in
    0) echo "Stage 0: skip_arm=1 — download_fw_nvram: enter_download_state (read-only) + BAR2 ioread32 probe + 442KB iowrite32 fw; sleeps 100 ms, reads host resetintr from fw->data, releases fw/NVRAM, returns -ENODEV." | tee -a "$LOG" ;;
...
echo "(test.176: test.175 proved msleep(100) after the complete fw write survives. This run keeps that sleep, then reads resetintr from host firmware memory before release/return. Still no post-write ARM probe, device resetintr use, NVRAM write, readback, or ARM release.)" | tee -a "$LOG"
echo "" | tee -a "$LOG"
...
if [ "$STAGE" -eq 0 ]; then
    SKIP_ARM=1
    WAIT_SECS=30  # test.176: host resetintr after post-fw msleep discriminator
else
    SKIP_ARM=0
    WAIT_SECS=60
fi
...
echo "" | tee -a "$LOG"
echo "=== Loading brcmfmac (bcm4360_reset_stage=$STAGE, bcm4360_skip_arm=$SKIP_ARM) --- test.176 ===" | tee -a "$LOG"
sync
