...
mkdir -p "$LOG_DIR"
LOG="$LOG_DIR/test.147.stage${STAGE}"

echo "=== test.147: skip early brcmf_dbg before pci_register_driver — stage=$STAGE ===" | tee "$LOG"
...
echo "(test.147: no new MMIO; skips brcmf_dbg in brcmf_pcie_register; test.145 buscore_reset ARM halt remains in place if reached)" | tee -a "$LOG"
echo "" | tee -a "$LOG"
...
if [ "$STAGE" -eq 0 ]; then
    SKIP_ARM=1
    WAIT_SECS=12  # test.147: registration-window markers + existing staged-reset/download diagnostics
else
    SKIP_ARM=0
    WAIT_SECS=35
fi
...
echo "" | tee -a "$LOG"
echo "=== Loading brcmfmac (bcm4360_reset_stage=$STAGE, bcm4360_skip_arm=$SKIP_ARM) --- test.147 ===" | tee -a "$LOG"
sync
