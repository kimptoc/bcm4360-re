...
## Current state (2026-04-24 22:15 BST, after T287b + full T288 static trace — **Major static progress on a no-fire day. fn@0x670d8 primary-source-confirmed as `si_doattach` (Broadcom SiliconInfo attach). fn@0x64590 confirmed as core enumerator with slot-indexed stride-4 stores. Per-slot core-id table at `[sched+slot*4+0xd4]`. For slot*4+0x214=0x258, slot=17 — math says PCIE2 lives at slot 17. Slot=17 claim remains LIVE until core-id independently verified. 0x83c (BCMA_CORE_PCIE2 standard ID) appears ZERO times in blob — BCM4360 uses a different PCIE2 core-id. PCIE2+0x100 is EP-side, NOT the RC-side RC_AXI_CONFIG; exact register name unknown. Artifacts: `phase6/t288_pcie2_reg_map.md`, `phase6/t288_fn670d8_trace.py`, `phase6/t288_fn64590.py`, `phase6/t288_verify_slot.py`, `phase6/t288_find_258_writers.py`. Ready to design T288a (read-only PCIE2 core probe) and T288b (per-slot core-id dump) after user's out-day. No fires today; user out tomorrow.**)
