...
## Current state (2026-04-27 20:42 BST — User picked DEFER fire. Two static surfaces resolved: A2-extension (gap writers, commit 49c3c35) + sched+0xCC writer (commit 5465446). Both are sched bookkeeping, no wake-question advance. Considering next free static surface before Q2 fire decision.
...
**Next move (advisor input 2026-04-27 20:42 BST).** Two safe-bet static surfaces closed = expected (both bookkeeping is the predicted result for safe bets). Not stuck — just been picking conservatively. **One more cheap surface justified — for upside, not safety.** Option 4 (readers of sched+0x358) reframed: not "find ALL readers", but **"is OOB pending read on a periodic schedule, or only on-dispatch from an upstream IRQ?"** That question discriminates wake-mechanism categories. After option 4 → cut to fire (B). While option 4 runs, draft B-style PRE-TEST plan in parallel — preferred variant: Phase 4B olmsg DMA (already plumbed at shared_info, never triggered) over MSI assert (needs new code). T303 FIRED at 20:10:56 BST (boot -1 uptime ~21 min — late but within row 83 clean window per row 83). All probe stages CLEAN through `t+90s SUMMARY count=2 sched_cc=0x1 events_p=0x18109000 pending=0x0` plus T303 readouts at every stage. **Boot -1 ended at 20:13:11** — silent kernel death right after t+90s SUMMARY, exact same [t+90s, t+120s] T270-BASELINE pattern (now n=7 without test300). Auto-recovery, NO SMC reset (boot 0 started 20:14:43). Current uptime ~2 min, lspci clean (MAbort-, CommClk+).
