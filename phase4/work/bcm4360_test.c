...
static int level3_tcm_and_fw(struct bcm4360_dev *dev)
{
	 ...
	 * Run BEFORE ARM halt and FW download because those can crash.
	 * Uses BAR0 backplane reads/writes only (no TCM access needed). */
	...
	{
		u32 srom_val;
		bp_write32(dev, CHIPCOMMON_BASE + CC_SROM_ADDR, 66);
		mdelay(1);
	...
	dev_info(&pdev->dev, "[level 3] === SROM write test complete ===\n");
	...
	if (max_level >= 5) {
		...
		dev_info(&pdev->dev, "[level 3] PASS — BAR2 mapped, SROM written, ready for level 5\n");
		return 0;
	}
