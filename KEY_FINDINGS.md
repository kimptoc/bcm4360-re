...
| **BIT_alloc (fn@0x9940 / fn@0x9944) reads PCIE2 core + 0x100 (absolute `0x18100100`), NOT chipcommon.** Register at this offset is NOT documented in upstream brcmfmac pcie.c/h — sits in gap between MAILBOXMASK (0x4C) and CONFIGADDR (0x120). Target of next static investigation. fn@0x9940 returns bits 0-4; fn@0x9944 (class 0x812) returns bits 8-12. | CONFIRMED (base) / LIVE (register identity) | T287b: `sched[+0x254/+0x258] = 0x18100000` post-set_active; T283 chain structure; pcie.h grep shows no 0x100 definition | 2026-04-24 |
| **Scheduler ctx runtime layout (primary-source, post-set_active):** `+0x10 = 0x00000011` (flag-like); `+0x18 = 0x58680001` (matches chipc.caps from T277/T278 console — proves readback infra works); `+0x88 / +0x8c = 0x18000000` (CHIPCOMMON MMIO base — different scheduler path than BIT_alloc); `+0x168 = 0x00000000`; `+0x254 / +0x258 = 0x18100000` (PCIE2 MMIO base). `+0x88` and `+0x254` are TWO independent MMIO-base fields for different scheduler subsystems — NOT a single chipcommon chain as T283 inferred. | CONFIRMED | phase5/logs/test.287b.journalctl.txt post-set_active row | 2026-04-24 |
...
| **`sched+0x258 = PCIE2 base` lands via slot-indexed store `[sched + slot*4 + 0x214]`**. Arithmetic: `slot*4 + 0x214 = 0x258` → **slot = 17**. If PCIE2's core-id matches slot 17's `[sched+slot*4+0xd4]` entry, the picture is complete. | LIVE (slot=17 claim) | T288 offset arithmetic; no independent core-id verification yet | 2026-04-24 |
| **`0x83c` (BCMA_CORE_PCIE2 standard core-id) does NOT appear anywhere in BCM4360 fw blob** (0 literal-pool hits, 0 disasm occurrences). BCM4360 either uses a different PCIE2 core-id or the enumerator matches by some other mechanism (e.g., table lookup). Chipcommon core-id `0x800` appears 8× in blob literal pool — confirms chipcommon still uses standard ID. | CONFIRMED | phase6/t288_verify_slot.py full blob scan | 2026-04-24 |
| **PCIE2+0x100 is the EP-side view of an undocumented register.** Upstream `bcma_driver_pcie2.h` names `0x100 = BCMA_CORE_PCIE2_RC_AXI_CONFIG` but that is root-complex-side; EP-side + 0x100 is NOT the same register. BIT_alloc reads 5 bits (bits 0-4 for fn@0x9940; bits 8-12 for fn@0x9944) — suggests an "interrupt pending/status" semantic on the EP-side. Exact register name unknown without datasheet. | LIVE | T288 analysis + bcma_driver_pcie2.h comparison | 2026-04-24 |
...
| **"The mask" in fn@0x1146C's dispatch was chipcommon-side per T283 — now requires revision.** T287b shows scheduler's BIT_alloc reads PCIE2, not chipcommon. fn@0x1146C's mask may also be PCIE2-side, not chipcommon. Write targets for the wake experiment should now be **PCIE2 core offsets** (starting with 0x100), not chipcommon INTMASK. | LIVE (strong inference, untested) | T287b correction to T283 | 2026-04-24 |
