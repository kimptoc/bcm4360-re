...
| **BIT_alloc (fn@0x9940 / fn@0x9944) reads chipcommon INTSTATUS at absolute `0x18000100`.** Access chain: `[scheduler_ctx+0x254]+0x100` where `[scheduler_ctx+0x254]` was set by class-0 thunk to point at CHIPCOMMON MMIO base. | SUPERSEDED | T287b primary-source runtime read: `sched_ctx+0x254 = 0x18100000` (PCIE2 base), NOT 0x18000000 (chipcommon). Correct target is PCIE2 core + 0x100 = abs `0x18100100`. Structure chain intact; base misidentified in T283. | 2026-04-24 (corrected from 2026-04-24 T283 static reading) |
| **BIT_alloc (fn@0x9940 / fn@0x9944) reads PCIE2 core + 0x100 (absolute `0x18100100`), NOT chipcommon.** | SUPERSEDED | `0x18100000` is CHIPCOMMON WRAPPER base, NOT PCIE2. See host-side `test.218` core enumeration in test.287b.journalctl.txt: chipcommon core[1] base=0x18000000 wrap=0x18100000; real PCIE2 is core[4] base=0x18003000. BIT_alloc actually reads CHIPCOMMON WRAPPER+0x100=`0x18100100`. | 2026-04-24 (superseded same day) |
| **BIT_alloc reads CHIPCOMMON WRAPPER + 0x100 (absolute `0x18100100`), returning 5-bit mask.** Real target is the AI/AXI backplane wrapper's register at offset 0x100 — likely OOB input status (interrupt-line routing into this agent). fn@0x9940 returns bits 0-4 (`and #0x1f`); fn@0x9944 returns bits 8-12 (lsr #8 then `and #0x1f`). Consistent with free-bit allocator scanning interrupt-routing lines. | CONFIRMED (base+offset) / LIVE (register semantics) | host-side test.218 core table + T287b runtime `sched+0x254/+0x258 = 0x18100000` + T283 arithmetic | 2026-04-24 |
...
| **`sched+0x258 = PCIE2 base` lands via slot-indexed store `[sched+slot*4+0x214]`** (slot=17 inferred from offset arithmetic). | SUPERSEDED | `0x18100000` is chipcommon WRAPPER, not PCIE2. Real PCIE2 is `0x18003000` (core[4] per test.218). `sched+0x258` is NOT populated via slot*4+0x214; writer path for +0x258 still open (not via enumerator's slot-indexed stores). | 2026-04-24 (superseded same day) |
...
| **BIT_alloc reads the AI-backplane wrapper register `oobselouta30` at offset 0x100 (absolute `0x18100100` for chipcommon's agent).** Documented in upstream kernel: `include/linux/bcma/bcma_regs.h:28` defines `BCMA_OOB_SEL_OUT_A30 = 0x0100` as "Agent registers (common for every core)"; `brcmsmac/aiutils.c:338` defines `struct aidmp { ... u32 oobselouta30; /* 0x100 */ ... }`. Register holds 4 OOB-routing fields packed 5/8-bit: fn@0x9940 extracts bits 4:0 (channel "30" route), fn@0x9944 extracts bits 12:8 (channel "74" route). Purpose: interrupt-line routing between backplane cores; BIT_alloc scans for unused routes to allocate new ISR callbacks. | CONFIRMED | asahi-linux kernel headers (explore agent research) + T283 disasm + BCM4360 test.218 core table | 2026-04-24 |
| **Every AI-backplane core has an identical "agent" wrapper register set** at its wrap base (chipcommon wrap=0x18100000, core[2] wrap=0x18101000, etc. per test.218). So BIT_alloc's `[sched+0x254]` can point at ANY core's wrapper depending on which class's scheduler node is active — not just chipcommon. For class=0 (pciedngl), `[sched+0x258] = 0x18100000` (chipcommon wrapper). Other classes would have different per-class wrapper bases stored at their respective table indices. | CONFIRMED-MECHANISM | AI backplane docs (aidmp struct) + test.218 wrap-column pattern (base+0x100000) | 2026-04-24 |
